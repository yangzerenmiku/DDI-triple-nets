...
def argument():
    ...
    parser.add_argument('--layer2_num_nodes', type=int, default=195)
    ...
    parser.add_argument('--layer3_num_nodes', type=int, default=600)
    parser.add_argument('--poly_d_dim', type=int, default=32)
    parser.add_argument('--test3_layer3_num_nodes', type=int, default=0)
    parser.add_argument('--test4_layer3_num_nodes', type=int, default=3826)
    parser.add_argument('--test5_layer3_num_nodes', type=int, default=297)
    parser.add_argument('--test_order', type=int, default=3)
    parser.add_argument('--K_Fold', type=int, default=5)
    parser.add_argument('--K_Fold_order', type=int, default=0)
    parser.add_argument('--use_ppi_neg', type=str2bool, default=True)
    parser.add_argument('--loss_name', type=str, default='CE')  ## 备选 'CE' 'LSCE' 'LSR'
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch%100 == 0:
                ...
                with torch.no_grad():
                    ...
                    args.test_order = 3
                    model_test.__prepare_test3_data__(args)
                    model_test.test(args, p_embedding, d_embedding)
                    _, _ = model_test.check()

                    args.test_order = 4
                    model_test.__prepare_test4_data__(args)
                    model_test.test(args, p_embedding, d_embedding)
                    _, _ = model_test.check()
